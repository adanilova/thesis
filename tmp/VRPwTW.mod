...
subject to BetweenStartEnd {k in VEHICLES}:					#(7) duration of each route should not exceed max route duration
	ATime["MoscowDepot",k]-DTime["MoscowDepot",k] <= maxduration[k] +
	M*(1 -CustAssigne["MoscowDepot",k]);

subject to DepartureTime {i in CUSTOMER, k in VEHICLES}:	 		#(8) departure time >= arrival time + service_time
	DTime[i,k] >= ATime[i,k]+WTime[i,k] + service_time[i] -
	M*(1-CustAssigne[i,k]);

subject to DepartureTime1 {i in CUSTOMER, k in VEHICLES}:			#(9) - Combined 12 and 12.5 enforce leaving of vehicle
	DTime[i,k] <= ATime[i,k] + WTime[i,k] + service_time[i] + 		#right after service is done. IMPORTANT.
		M*(1-CustAssigne[i,k]);

subject to ArrivalTime {(i,j) in ARC, k in VEHICLES}:				#(10) arrival t at customer j = departure t from customer i +
	ATime [j,k] >= DTime[i,k] + travel_time[i,j] -				# + travel time from i to j only if arc assigned to vehicle k
	M*(1-X[i,j,k]);

subject to ArrivalTime1 {(i,j) in ARC, k in VEHICLES}:				#(11) Complementary conditions for #(10)
	ATime [j,k] <= DTime[i,k] + travel_time[i,j] +
	M*(1-X[i,j,k]);

subject to STW {i in NODE, k in VEHICLES, p in 1..13}:				#(12) arrival time + waiting time >= start TW, only if
...
subject to ETW {i in NODE, k in VEHICLES, p in CUSTTIMEWINDOWS[i]}:		#(13) arrival time + waiting time <= end TW, only if
	ATime[i,k] + WTime [i,k]+service_time[i] <= timewind[i,2,p] +		# customer i is assigned to vehicle k and TW p is chosen
	M*(1-CustAssigne[i,k]) + M*(1-Served[i,p]);

subject to visit2 {i in CUSTOMER}:						#(14) one TW for each customer
	sum{ p in CUSTTIMEWINDOWS[i], k in VEHICLES}Served[i,p]*CustAssigne[i,k]=1;

subject to visit{k in VEHICLES}:						#(15) one TW for each customer
	sum{ p in 1..13}Served["MoscowDepot",p]*CustAssigne["MoscowDepot",k]<=2;

subject to Serve_when_available {i in NODE, p in 1..13}:			#(16)
	Served[i,p]<=service_available [i,p];

#subject to I_whip_my_slaves_back_and_forth {p in 1..13, k in VEHICLES}:	#(17) ensure that sum of travel and service time on any day is
...
subject to VehicleRouteDuration {k in VEHICLES}:				#(21) total route travel time
