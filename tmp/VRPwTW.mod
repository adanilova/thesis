...
param l;									# =1 for start of the TW, =2 for end of the TW for all customer i
...
param timewind{i in CUSTOMER, l in 1..2, p in CUSTTIMEWINDOWS[i]};		# every start/end TW for every customer in every day per route
...
subject to LOL {i in NODE}:							##4 every customer i visited by 1 vehicle
	sum {k in VEHICLES, j in NODE} x[i,j,k] <=1;

subject to LOL1 {i in NODE}:							##5 every customer j visited by 1 vehicle
...
subject to STW {i in CUSTOMER, l in 1..2, p in CUSTTIMEWINDOWS[i], k in VEHICLES}:	##15 arrival time + waiting time >= start TW, only if
...
subject to ETW {i in CUSTOMER,l in 1..2,  p in CUSTTIMEWINDOWS[i], k in VEHICLES}:	##16 arrival time + waiting time <= end TW, only if
