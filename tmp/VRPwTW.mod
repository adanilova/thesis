...
subject to Input_Output_Balance2 {i in NODE, k in VEHICLES}:										#(3) if one vehicle come to node - one income
	sum {(i,j) in ARC:i<>j} X[i,j,k] = sum {(j,i) in ARC:i<>j} X[j,i,k];

subject to Traverse {k in VEHICLES, (i,j) in ARC}:															#(4) any ARC (i,j) can be traversed by vehicle k if both CustAssigne = 1
	2 * X [i,j,k] <= CustAssigne [i,k] + CustAssigne [j,k];

subject to CustVisit {i in CUSTOMER}:						        												#(5) every customer i visited by 1 vehicle
	sum {k in VEHICLES, (i,j) in ARC:i<>j} X[i,j,k] <=1;

subject to CustVisit1 {i in CUSTOMER}:						        											#(6) every customer i visited by 1 vehicle
	sum {k in VEHICLES, (i,j) in ARC:i<>j} X[j,i,k] <=1;

subject to Loading {k in VEHICLES}:																							#(7) capacity should not be exeed
	sum {i in CUSTOMER, (i,j) in ARC} demand[i] * X[i,j,k] <= capacity[k];

subject to BetweenStartEnd {k in VEHICLES}:																			#(8) duration of each route should not exceed max route duration
	ATime["MoscowDepot",k]-DTime["MoscowDepot",k] <= maxduration[k] +
	M*(1 -CustAssigne["MoscowDepot",k]);

subject to DepartureTime {i in CUSTOMER, k in VEHICLES}:												#(9) departure time >= arrival time + service_time
	DTime[i,k] >= ATime[i,k]+WTime[i,k] + service_time[i] -
	M*(1-CustAssigne[i,k]);

subject to DepartureTime1 {i in CUSTOMER, k in VEHICLES}:												#(10) - Combined 12 and 12.5 enforce leaving of vehicle
	DTime[i,k] <= ATime[i,k] + WTime[i,k] + service_time[i] + 										#right after service is done. IMPORTANT.
		M*(1-CustAssigne[i,k]);

subject to ArrivalTime {(i,j) in ARC, k in VEHICLES}:														#(11) arrival t at customer j = departure t from customer i +
	ATime [j,k] >= DTime[i,k] + travel_time[i,j] -																# + travel time from i to j only if arc assigned to vehicle k
	M*(1-X[i,j,k]);

subject to ArrivalTime1 {(i,j) in ARC, k in VEHICLES}:													#(12) same with 11
	ATime [j,k] <= DTime[i,k] + travel_time[i,j] +
	M*(1-X[i,j,k]);

subject to STW {i in NODE, k in VEHICLES, p in 1..13}:													#(13) arrival time + waiting time >= start TW, only if
...
subject to ETW {i in NODE, k in VEHICLES, p in CUSTTIMEWINDOWS[i]}:							#(14) arrival time + waiting time <= end TW, only if
	ATime[i,k] + WTime [i,k]+service_time[i] <= timewind[i,2,p] +									# customer i is assigned to vehicle k and TW p is chosen
	M*(1-CustAssigne[i,k]) + M*(1-Served[i,p]);

subject to visit2 {i in CUSTOMER}:																							#(15) one TW for each customer
	sum{ p in CUSTTIMEWINDOWS[i], k in VEHICLES}Served[i,p]*CustAssigne[i,k]=1;

	subject to visit{k in VEHICLES}:																							#(16) one TW for each customer
	sum{ p in 1..13}Served["MoscowDepot",p]*CustAssigne["MoscowDepot",k]<=2;

subject to Serve_when_available {i in NODE, p in 1..13}:												#(17)
	Served[i,p]<=service_available [i,p];

#subject to I_whip_my_slaves_back_and_forth {p in 1..13, k in VEHICLES}:				#(18) ensure that sum of travel and service time on any day is
	#sum {i in CUSTOMER} (service_time[i]*Served[i,p]) 														# less than working limit of a driver
	#+ sum {(i,j) in ARC} (X[i,j,k]*Served[i,p]*travel_time[i,j])
	#<= working_limit;

subject to vehicleassign{i in NODE, k in VEHICLES }:														#(19) ensure that customer i is served by vehicle k only if it's used
	VehUsed[k] >= CustAssigne[i,k];

#subject to WStart_Time {k in VEHICLES}:																				#(20) start waiting time is 0
	#WTime ["MoscowDepot",k] = 0;

subject to DepartureTimefromDepot{k in VEHICLES}:																#(21) departure time from depot is 0
	DTime ["MoscowDepot",k] = 0;

################################################################################

subject to VehicleRouteDuration {k in VEHICLES}:																#(22) total route travel time
