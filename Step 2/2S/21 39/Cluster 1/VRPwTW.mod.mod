...
param dayworkD {i in CUSTOMER};
param dayworkU {i in CUSTOMER};
...
subject to STW {i in CUSTOMER, l in 1..L, p in CUSTTIMEWINDOWS[i], k in VEHICLES: p>dayworkD[i]and p<dayworkU[i]  }:	##15 arrival time + waiting time >= start TW, only if
...
subject to ETW {i in CUSTOMER,l in 1..L,  p in CUSTTIMEWINDOWS[i], k in VEHICLES:p>dayworkD[i]and p<dayworkU[i]  }:	##16 arrival time + waiting time <= end TW, only if
